...
export function likeComment(feedItemId, commentIndex, user, cb){
  var feedItem = readDocument('feedItems',feedItemId);

  feedItem.comments[commentIndex].likeCounter.push(user);

  writeDocument('feedItems',feedItem);

  emulateServerReturn(feedItem.comments[commentIndex].likeCounter.map((id)=>readDocument('users',id)),cb);
}
